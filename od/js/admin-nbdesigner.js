...
var NBDESIGNADMIN = {
    ...
    resetBoxes: function (command) {
        ...
        jQuery.each(jQuery(index + ' .nbdesigner-box-container'), function (key, val) {
            ...
            jQuery(this).find('.use_svg_overlay').attr('name', name + '[' + key + '][use_svg_overlay]');
            jQuery(this).find('.design_svg_overlay').attr('name', name + '[' + key + '][design_svg_overlay]');
            jQuery(this).find('.cutline_svg_overlay').attr('name', name + '[' + key + '][cutline_svg_overlay]');
            jQuery(this).find('.safezone_svg_overlay').attr('name', name + '[' + key + '][safezone_svg_overlay]');
        });
    ...
    toggleSvgOverlay: function(e){
        jQuery(e).parents('.nbdesigner-box-collapse').find('.svg-overlay-toggle').toggle();
    },
    ...
    validateSvgOverlay: function(e){
        var value = jQuery(e).val();
        value = value.replace(/<!--[\s\S]*?-->/g, "").replace(/<\?xml[\s\S]*?\?>/g, "")
            .replace(/<style[\s\S]*?style>/g, "")
            .replace(/\n|\r/gm, " ")
            .replace( /  /g, " " )
            .replace(/^ /g, "")
            .replace( / $/g, "" )
            .replace(/(?:class|id|style|xmlns\:xlink|xml\:space|fill|vector-effect)=['\"][^'\"]*['\"]/g, "" )
            .replace(/<\/?g>/g , "")
            .replace(/\s{2,}/g, ' ')
            .replace(/(\/?>) </g, '$1<')
