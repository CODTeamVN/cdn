...
window.onSavedDesign = function(path, uuid, numberOfSide) {
  const event = new CustomEvent("onSavedDesign", {
    detail: { path, uuid, numberOfSide },
  });
  ...
  const imgEls = [...Array(numberOfSide).keys()].map((key) => `<img class="od-preview" src="${owi.owiAssetUrl}/${path}frame_${key}.png" />`);
...
window.initDesignEditor = function(actionUrl) {
  ...
  const odHtml = `
    <form action="${actionUrl}" target="od-editor" method="post" id="editor-form">
      <input type="hidden" name="X-API-KEY" value="${owi.owiApiKey}" />
    </form>
...
window.openEditDesign = function(designUUID) {
  const actionUrl = `${owi.owiApiUrl}/edit/design/${designUUID}`;
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("owi_width") && document.getElementById("owi_height")) {
    function updateDimension() {
      window.updatePrintOptions("dimension", {
        width: parseInt(document.getElementById("owi_width").value),
        height: parseInt(document.getElementById("owi_height").value),
      });
    }

    updateDimension();

    document.getElementById("owi_width").addEventListener("change", updateDimension);
    document.getElementById("owi_height").addEventListener("change", updateDimension);
  }
  ...
  if (document.getElementById("od-wrapper")) {
    ...
    window.initDesignEditor(`${owi.owiApiUrl}/product?${query}`);
  ...
  if (document.querySelectorAll(".od-cart-preview").length) {
    ...
    [].forEach.call(divs, function(div) {
      ...
      fetch(`${owi.owiApiUrl}/designs/publish/${uuid}`, {
        method: "GET",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
          Authorization: `Bearer ${owi.owiApiKey}`,
        },
      })
        ...
        .then(function(response) {
          if (response.path) {
            ...
            for (let i = 0; i < response.numberOfSide; i++) {
              images += `<img class="od-preview" style="" src="${owi.owiAssetUrl}/${response.path}frame_${i}.png" />`;
            }
...
window.downloadOdDesign = function(uuid) {
  ...
  let result = {};
  document.getElementById(`owi-download-${uuid}`).insertAdjacentHTML("afterend", `<div class="od-export-loader" id="loader-${uuid}"></div>`);
  fetch(`${owi.owiApiUrl}/resources/shop/pdf/${uuid}`, {
    method: "GET",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
    ...
    .then(function(response) {
      ...
      async function fetchImages() {
        const [pngResponse, jpgResponse, epsResponse] = await Promise.all(
          ["png", "jpg", "eps"].map((type) => {
            return fetch(`${owi.owiApiUrl}/resources/shop/image/${uuid}/${type}`, {
              method: "GET",
              headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
      ...
      `;
          ...
          document.getElementById(`owi-download-${uuid}`).classList.toggle("od-none");
