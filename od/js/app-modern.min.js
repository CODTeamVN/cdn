...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.insertPartGlobalTemplate = function(id, $index){
        var stage = $scope.stages[$scope.currentStage],
        _canvas = stage['canvas'],
        layers = _canvas.getObjects();
        if( layers.length > 0 ){
            $scope._currentGlobalTempId = id;
            jQuery('.nbd-popup.insert-part-global-template-alert').nbShowPopup();
        }else{
            $scope.insertGlobalTemplate(id, $index);
        }
    };
    $scope.insertGlobalTemplate = function(id, $index){
        ...
        $scope.closePopupInsertGlobalPartTem();
        $scope.showDesignTab();
        $scope.clearHistory();
        $scope.toggleStageLoading();
        $scope.getGlobalTemplate(id, function(data){
            ...
            //var stageIndex = 0;
            var stageIndex = $scope.currentStage;
            ...
            function loadStage(stageIndex){
                if( stageIndex > $scope.currentStage ){
                    afterLoadStage();
                    return;
                }
...
function receiveMessage(event){
    if( event.origin == window.location.origin ){
        ...
        if( typeof event.data == 'object' && event.data.type == 'taked_screenshot' ){
