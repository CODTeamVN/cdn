...
const loadSvgFont = async (url, callback) => {
    try {
        const buf = await fetchFont(url);
        const fontJson = opentype.parse(Module.decompress(buf));
        callback(false, fontJson);
    } catch (err) {
        callback(true, null);
    }
};
const designUuid = function(a,b){for(b=a='';a++<36;b+=a*51&52?(a^15?8^Math.random()*(a^20?16:4):4).toString(16):'-');return b};
...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'filterFontFilter', 'filterArtFilter', 'filterBackgroundFilter', '$timeout', '$http', '$document', '$interval',
    function($scope, FabricWindow, $window, filterFontFilter, filterArtFilter, filterBackgroundFilter, $timeout, $http, $document, $interval){
    $scope.stages = [];
    var d = new Date(),
    t = d.getTime();
    ...
    $scope.initSettings = async function(){
        $scope.debugMode = appConfig.debugMode;
        $scope.the_first_time_load_page = true;
        $scope.printingOptionsAvailable = true;

        if( NBDESIGNCONFIG.task == 'create' ){
            NBDESIGNCONFIG.product_data = {
                option: {
                    admindesign: false,
                    templateCategories: [],
                    dpi: NBDESIGNCONFIG.nbdesigner_default_dpi
                },
                product: [
                    {
        ...
        if( NBDESIGNCONFIG.task == 'edit' ){
            $scope.templateSize = {
                width: NBDESIGNCONFIG.product_data.product[0].product_width,
                height: NBDESIGNCONFIG.product_data.product[0].product_height
            };
            NBDESIGNCONFIG.product_data.option.dpi = NBDESIGNCONFIG.nbdesigner_default_dpi;
        }

        angular.copy(NBDESIGNCONFIG, $scope.settings);
        angular.extend($scope.settings, {
            showRuler: checkMobileDevice() ? false : NBDESIGNCONFIG.nbdesigner_show_ruler,
            showGrid: NBDESIGNCONFIG.nbdesigner_show_grid,
            showDimensions: NBDESIGNCONFIG.nbdesigner_show_product_dimensions,
            bleedLine: !$scope.isTemplateMode ? NBDESIGNCONFIG.nbdesigner_show_bleed : false,
            snapMode: {status: false, type: 'layer'},
            showWarning: {
                oos: NBDESIGNCONFIG.nbdesigner_show_warning_oos,
                ilr: NBDESIGNCONFIG.nbdesigner_show_warning_il
            },
            is_mobile:  checkMobileDevice(),
            active_design: checkMobileDevice() ?  true : false
        });

        // if( $scope.isTemplateMode ) $scope.settings.nbdesigner_dimensions_unit = 'px';

        $scope.settings.active_template = (!$scope.settings.active_design && !NBDESIGNCONFIG.nbdesigner_hide_template_tab && (NBDESIGNCONFIG.product_data["option"]['admindesign'] || $scope.isTemplateMode)) ? true : false;
        $scope.settings.active_typos = (!$scope.settings.active_design && !$scope.settings.active_template && NBDESIGNCONFIG.nbdesigner_enable_text) ? true : false;
        $scope.settings.active_backgrounds = (!$scope.settings.active_design && !$scope.settings.active_template && !$scope.settings.active_typos && NBDESIGNCONFIG.nbdesigner_show_background_tab) ? true : false;
        $scope.settings.active_cliparts = (!$scope.settings.active_design && !$scope.settings.active_template && !$scope.settings.active_typos && !$scope.settings.active_backgrounds && NBDESIGNCONFIG.nbdesigner_enable_clipart) ? true : false;
        $scope.settings.active_photos = (!$scope.settings.active_design && !$scope.settings.active_template && !$scope.settings.active_typos && !$scope.settings.active_backgrounds && !$scope.settings.active_cliparts && NBDESIGNCONFIG.nbdesigner_enable_image) ? true : false;

        $scope.settings.show_elements_tab = (!(NBDESIGNCONFIG.nbdesigner_enable_clipart || NBDESIGNCONFIG.nbdesigner_enable_qrcode || NBDESIGNCONFIG.nbdesigner_enable_draw) || NBDESIGNCONFIG.nbdesigner_hide_element_tab) ? false : true;
        $scope.settings.active_elements = (!$scope.settings.active_design && !$scope.settings.active_template && !$scope.settings.active_typos && !$scope.settings.active_backgrounds && !$scope.settings.active_cliparts && !$scope.settings.active_photos && $scope.settings.show_elements_tab && !NBDESIGNCONFIG.nbdesigner_hide_element_tab) ? true : false;
        $scope.settings.active_layers = (!$scope.settings.active_design && !$scope.settings.active_template && !$scope.settings.active_typos && !$scope.settings.active_backgrounds && !$scope.settings.active_cliparts && !$scope.settings.active_photos && !$scope.settings.active_elements && !NBDESIGNCONFIG.nbdesigner_hide_layer_tab) ? true : false;
        
        $scope.offsetDesignWrap = NBDESIGNCONFIG.nbdesigner_show_design_border ? -1 : 0;
        ...
        $scope.currentColorInfo = {
            rgb: NBDESIGNCONFIG.nbdesigner_default_color,
            c: 100,
            m: 100,
            y: 100,
            k: 1
        ...
        $scope.forceMinSize = $scope.settings.nbdesigner_force_min_font_size;
        $scope.forceMaxSize = $scope.settings.nbdesigner_force_max_font_size;
        ...
        $scope.resource = {
            ...
            drawMode: {status: false, brushWidth: 1, brushType: 'Pencil', brushColor: NBDESIGNCONFIG.nbdesigner_default_color, showColorPicker: false},
            ...
            clipart: {filter: {perPage: 20, currentPage: 1, total: 0, currentCat: {}}, data: [], onload: false, init: true, fetched: false, filteredArts : []},
            background: {filter: {perPage: 21, currentPage: 1, total: 0, currentCat: {}}, data: [], onload: false, init: true, fetched: false, filteredBackgrounds : []},
            font: {filter: {perPage: 25, currentPage: 1, total: 0}, data: [], filteredFonts : []},
            ...
            shape: {filter: {perPage: 20, currentPage: 1, totalPage: 0}, data: [], init: true, onload: false, filteredItems: [], first: true},
            icon: {filter: {perPage: 20, currentPage: 1, totalPage: 0, currentCat: null}, data: [], cat: [], init: true, onload: false, first: true, filteredItems: []},
            ...
            line: {filter: {perPage: 20, currentPage: 1, totalPage: 0}, data: [], init: true, onload: false, filteredItems: [], first: true},
            globalTemplate: {filter: {perPage: 20, currentPage: 1, totalPage: 0}, data: [], init: true, onload: false},
            templates: {filter: {perPage: 50, currentPage: 1, totalPage: 0, currentCat: null, search: ''}, items: [], categories: [], init: true, onload: false, fetched: false},
            templateType: 'custom',
            mockups: [],
            canAdd: true,
            ...
            shareDesign: false,
            products: [],
            savedDesign: NBDESIGNCONFIG.task == 'edit-design' ? true : false,
            designUUID: NBDESIGNCONFIG.task == 'edit-design' ? NBDESIGNCONFIG.currentTemplate.uuid : '',
            templateColumn: 1
        };

        $scope.resource.defaultPalette = [];
        NBDESIGNCONFIG.colorPalettes.forEach(cat => {
            if( cat.colors.length ){
                const colors = [];
                cat.colors.forEach(color => {
                    colors.push(color.rgb.toLowerCase());
                    $scope.listInfoAddedColor.push({
                        rgb: color.rgb.toLowerCase(),
                        c: color.c,
                        m: color.m,
                        y: color.y,
                        k: color.k,
                ...
                $scope.resource.defaultPalette.push(colors);
            }
        });
        $scope.verticalText = {
            active: false,
        ...
        if( ($scope.resource.templates.length == 0 || NBDESIGNCONFIG.product_data.option.admindesign != true || ( !$scope.isTemplateMode && $scope.settings.nbdesigner_hide_template_tab ) ) &&  $scope.settings.nbdesigner_enable_text ){
            $scope.getResource('typography', '#tab-typography');
        }
        if( NBDESIGNCONFIG.product_data.option.admindesign != true && !$scope.settings.nbdesigner_enable_text && $scope.settings.nbdesigner_show_background_tab ){
            $scope.getResource('background', '#tab-background', true);
        };
        if( NBDESIGNCONFIG.product_data.option.admindesign != true && !$scope.settings.nbdesigner_enable_text && !$scope.settings.nbdesigner_show_background_tab && $scope.settings.nbdesigner_enable_clipart ){
            $scope.getResource('clipart', '#tab-svg', true);
        };

        if($scope.isTemplateMode || (NBDESIGNCONFIG.product_data.option.admindesign && NBDESIGNCONFIG.product_data.option.templateCategories.length)){
            $scope.loadTemplates(true);
        }
        ...
        $scope.$watchCollection('resource.background.filter', function(newVal, oldVal){
            $scope.resource.background.filteredBackgrounds = filterBackgroundFilter($scope.resource.background.data.backgrounds, $scope.resource.background.filter);
            if(newVal.search != oldVal.search){
                $timeout(function() {
        ...
        $scope.settings.dynamicStage = ( angular.isDefined( $scope.settings.product_data.option.dynamic_side ) && $scope.settings.product_data.option.dynamic_side == true && !$scope.isTemplateMode ) ? true : false;
        ...
        $scope.settings.enableImageFilter = NBDESIGNCONFIG.nbdesigner_modern_layout_image_filter;
        ...
        const getGoogleFonts = new Promise((resolve, reject) => {
            $http({
                method: 'GET',
                url: NBDESIGNCONFIG.googleFontsUrl,
            }).then(function successCallback(response){
                if( angular.isDefined( response.data ) ){
                    resolve(response.data)
                }
            }, function errorCallback(response) {
                console.log('Fail to load google fonts!');
                reject(response)
            });
        });

        const getFonts = new Promise((resolve, reject) => {
            $http({
                method: 'GET',
                url: `${NBDESIGNCONFIG.apiUrl}/resources/fonts`,
                headers: {
                    Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`,
                    'Accept': 'application/json'
                }
            }).then(function successCallback(response){
                if( angular.isDefined( response.data ) ){
                    resolve(response.data.fonts);
                }
            }, function errorCallback(response) {
        ...
        await Promise.all([getGoogleFonts, getFonts]).then(([googleFonts, fonts]) => {
            NBDESIGNCONFIG.googleFonts = googleFonts;
            $scope.resource.font.data = [...$scope.resource.font.data, ...fonts];
            $scope.resource.font.filter.total = $scope.resource.font.data.length;
            $scope.resource.font.filteredFonts = filterFontFilter($scope.resource.font.data, $scope.resource.font.filter);
        }, function (err) {
            console.log(err)
    ...
    $scope.searchTemplate = function(){
        $scope.resource.templates.items = [];
        $scope.resource.templates.filter.totalPage = 0;
        $scope.resource.templates.filter.currentPage = 1;
        $scope.loadTemplates();
    };
    ...
    $scope.init = function() {
        ...
        nbd_window.postMessage("onLoadedEditor", '*');

        $scope.isTemplateMode = NBDESIGNCONFIG.task === 'create' || NBDESIGNCONFIG.task == 'edit';
        $scope.initSettings();
    ...
    $scope._localStorage = {
        insert: function(type, obj) {
            const storedObjects = this.get(type);
            storedObjects.push(obj);
            if( type == 'od-uploaded') {
                if( storedObjects.length > 20 ) storedObjects.shift();
            }
            localStorage.setItem(type, JSON.stringify(storedObjects));
        },
        ...
        get: function(type){
            ...
            if( data ){
                if( type != 'od-uploaded') return JSON.parse(data);
                let images = JSON.parse(data);
                const expired = new Date().getTime() - 30 * 24 * 60 * 60 * 1e3;
                images = images.filter(image => image.createdAt > expired);
                this.save(type, JSON.stringify(images));
                return images;
            }else{
                return [];
            }
    ...
    $scope.getResource = function(type, container, callback){
        if( type == 'typography' && $scope.settings.nbdesigner_hide_typo_section ) return;
        ...
        if( type == 'typography' ){
            $http({
                method: 'GET',
                url: `${NBDESIGNCONFIG.typographyUrl}typography.json`,
            }).then(function successCallback(response){
                if( angular.isDefined( response.data ) ){
                    $scope.resource[type].data = response.data.data; 
        ...
        if( type == 'clipart' ){
            if( $scope.resource[type].fetched ){
                jQuery(container + ' .loading-photo').hide();
                return;
            }
            $http({
                method: 'GET',
                url: `${NBDESIGNCONFIG.apiUrl}/resources/cliparts`,
                headers: {
                    Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`,
                    'Accept': 'application/json'
                }
                ...
                if( angular.isDefined( response.data ) ){
                    $scope.resource[type].fetched = true;
                    $scope.resource[type].data.arts = response.data.cliparts;
                    $scope.resource[type].data.cat = response.data.clipartCategories;
                    $scope.resource[type].filter.total = $scope.resource[type].data.arts.length;
                }
                jQuery(container + ' .loading-photo').hide();
                if( callback ) $scope.afterGetResource(type);
            }, function errorCallback(response) {
                jQuery(container + ' .loading-photo').hide();
                console.log('Fail to load cliparts!', response);
            });
        }
        ...
        if( type == 'background' ){
            ...
            $http({
                method: 'GET',
                url: `${NBDESIGNCONFIG.apiUrl}/resources/backgrounds`,
                headers: {
                    Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`,
                    'Accept': 'application/json'
                }
                ...
                if( angular.isDefined( response.data ) ){
                    $scope.resource[type].fetched = true;
                    $scope.resource[type].data.backgrounds = _.reverse(response.data.backgrounds);
                    $scope.resource[type].data.cat = response.data.backgroundCategories;
                    $scope.resource[type].filter.total = $scope.resource[type].data.backgrounds.length;
                }
                jQuery(container + ' .loading-photo').hide();
                if( callback ) $scope.afterGetResource(type);
            }, function errorCallback(response) {
                jQuery(container + ' .loading-photo').hide();
                console.log('Fail to load backgrounds!', response);
            });
        }
    };
    $scope.onClickTab = function(type, tab){
        ...
        switch(type){
            ...
            case 'url':
                if( type == 'upload' ){
                    if( $scope.settings.nbdesigner_cache_uploaded_image && $scope.resource.upload.data.length == 0){
                        $scope.resource.upload.data = $scope._localStorage.get('od-uploaded');
                    };
                };
                $timeout(function(){
    ...
    $scope.loadTemplates = function(first){
        const apiUrl = first ? `${NBDESIGNCONFIG.apiUrl}/resources/templates` : `${NBDESIGNCONFIG.apiUrl}/templates/shop`;
        let query = first ? {
            getAllCategories: $scope.isTemplateMode ? true : false,
            templateCategories: $scope.isTemplateMode ? [] : $scope.settings.product_data.option.templateCategories
        } : {
            page: $scope.resource.templates.filter.currentPage,
            limit: $scope.resource.templates.filter.perPage,
            filters: []
        };

        if( !first ){
            if( $scope.resource.templates.filter.search ){
                query.filters = [{
                    "field": "name",
                    "operator": "contain",
                    "value": $scope.resource.templates.filter.search
                }];
            }
            if( $scope.resource.templates.filter.currentCat &&  $scope.resource.templates.filter.currentCat.id != -1 ){
                query.filters.push({
                    "field": "categories",
                    "operator": "=",
                    "value": $scope.resource.templates.filter.currentCat.id
                });
            } else {
                if(!$scope.isTemplateMode) query.filters.push(...($scope.settings.product_data.option.templateCategories.map(cat => {
                    return {
                        field: 'categories',
                        operator: "=",
                        value: cat,
        ...
        $http({
            ...
            if( angular.isDefined( response.data ) ){
                if( first ){
                    $scope.resource.templates.fetched = true;
                    $scope.resource.templates.categories = response.data.categories;
                }
                $scope.resource.templates.items.push(...(first ? response.data.templates : response.data.items));
                $scope.resource.templates.filter.totalPage = Math.ceil(response.data.total / $scope.resource.templates.filter.perPage);

                if( $scope.resource.templates.filter.currentCat && $scope.resource.templates.filter.currentCat.templateColumn ){
                    $scope.resource.templateColumn = $scope.resource.templates.filter.currentCat.templateColumn;
                } else {
                    if( Array.isArray($scope.resource.templates.categories) && $scope.resource.templates.categories.length ){
                        $scope.resource.templateColumn = $scope.resource.templates.categories[0].templateColumn || 1;
                    }
                }
    ...
    $scope.toggleTemplateColumn = function(){
        if( $scope.resource.templateColumn == 3 ){
            $scope.resource.templateColumn = 1;
        } else {
            $scope.resource.templateColumn += 1;
        }
    };
    ...
    $scope.changeTemplateDimension = function(){
        $scope.templateSize.width = $scope.templateSize.width != '' ? $scope.templateSize.width : 5;
        $scope.templateSize.height = $scope.templateSize.height != '' ? $scope.templateSize.height : 5;
        $scope.templateSize.width = $scope.templateSize.width <= 1e3 ? $scope.templateSize.width : 1e3;
        $scope.templateSize.height = $scope.templateSize.height <= 1e3 ? $scope.templateSize.height : 1e3;

        $scope.saveDesign();
        angular.forEach($scope.settings.product_data.product, function(side, key){
            side.product_height =  $scope.templateSize.height;
            side.product_width = $scope.templateSize.width;
            side.real_width = $scope.templateSize.width;
            side.real_height =  $scope.templateSize.height;
        });
    ...
    $scope.selectStagePreview = function($event, stageIndex){
        $scope.switchStageTo(stageIndex, 'left-right');
        var scrollLeft = jQuery('.nbd-stages-nav-inner-extra').scrollLeft();
        setTimeout(function(){
            jQuery('.nbd-stages-nav-inner-extra').animate({scrollLeft: scrollLeft + 100}, 300);
        });
    };

    $scope.getMedia = async function(type, context){
        ...
        if( $scope.resource.element.type != type || context == 'search' ){
            ...
            $scope.resource[type].filter.currentPage = 1;
            if( type == 'flaticon' || type == 'storyset' ){
                $scope.resource[type].data = [];
            } else {
                $scope.resource[type].filteredItems = [];
            }
        };
        $scope.resource.element.type = type;
        ...
        switch( type ){
            case 'storyset':
                $http({
                    ...
                    _.each(data.data, function(item, key) {
                        $scope.resource[type].data.push({
                            url: `${NBDESIGNCONFIG.apiUrl}/files/shop/proxy?token=${NBDESIGNCONFIG.clientToken}&imageUrl=${item.src}`,
                ...
                break;
            default:
                if($scope.resource[type].first) {
                    const getCliparts = new Promise((resolve, reject) => {
                        setTimeout(() => {
                            $http({
                                method: 'GET',
                                url: NBDESIGNCONFIG.globalCliparts,
                            }).then(function successCallback(response){
                                if( angular.isDefined( response.data ) ){
                                    resolve(response.data)
                                }
                    ...
                    const getClipartCategories = new Promise((resolve, reject) => {
                        setTimeout(() => {
                            $http({
                                method: 'GET',
                                url: NBDESIGNCONFIG.globalClipartCategories,
                            }).then(function successCallback(response){
                                if( angular.isDefined( response.data ) ){
                                    resolve(response.data)
                                }
                    ...
                    await Promise.all([getCliparts, getClipartCategories]).then(([items, categories]) => {
                        $scope.resource['icon'].first = false;
                        $scope.resource['shape'].first = false;
                        $scope.resource['line'].first = false;

                        $scope.resource['icon'].cat = categories;
                        $scope.resource.icon.filter.currentCat = _.filter($scope.resource.icon.cat, { id: '' + category })[0];

                        _.each(items, function(item, key) {
                            $scope.resource['icon'].data.push({
                                url: NBDESIGNCONFIG.globalClipartPrefix + item.id + '.svg',
                                name: item.name || '',
                                cate_id: item.cate_id
                            });
                        });

                        const lineItems = items.filter(item => item.cate_id == 78);
                        const shapeItems = items.filter(item => item.cate_id == 66);

                        _.each(lineItems, function(item, key) {
                            $scope.resource['line'].data.push({
                                url: NBDESIGNCONFIG.globalClipartPrefix + item.id + '.svg',
                                name: item.name || '',
                            });
                        });
                        ...
                        _.each(shapeItems, function(item, key) {
                            $scope.resource['shape'].data.push({
                                url: NBDESIGNCONFIG.globalClipartPrefix + item.id + '.svg',
                                name: item.name || '',
                            });
                        });
                ...
                if( type == 'icon' ){
                    const currentItems = search != '' ? 
                        $scope.resource[type].data.filter(item => item.name.includes(search)) : 
                        $scope.resource[type].data.filter(item => item.cate_id == category);
                    $scope.resource[type].filter.totalPage = Math.ceil(currentItems.length / $scope.resource[type].filter.perPage);
                    $scope.resource[type].filteredItems.push(...currentItems.slice(
                        ($scope.resource[type].filter.currentPage - 1) * $scope.resource[type].filter.perPage,
                        $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage
                    ));
                } else {
                    const currentItems = $scope.resource[type].data.filter(item => item.name.includes(search));
                    $scope.resource[type].filter.totalPage = Math.ceil(currentItems.length / $scope.resource[type].filter.perPage);
                    $scope.resource[type].filteredItems.push(...currentItems.slice(
                        ($scope.resource[type].filter.currentPage - 1) * $scope.resource[type].filter.perPage,
                        $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage
                ...
                setTimeout(() => {
                    jQuery('#tab-element .loading-photo').hide();
                }, 1e3)
                break;
        }
    };
        ...
        switch(type){
            case 'clipart':
                ...
                _.each($scope.resource.clipart.data.arts, function(art, k) {
                    art.url = `${NBDESIGNCONFIG.baseUrl}/${art.url}`;
                });
            ...
            case 'background':
                ...
                _.each($scope.resource.background.data.backgrounds, function(background, k) {
                    background.url = `${NBDESIGNCONFIG.baseUrl}/${background.url}`;
                    background.preview = `${NBDESIGNCONFIG.baseUrl}/${background.preview}`;
                });
                _.each($scope.resource.background.data.cat, function(cat, key) {
                    cat.amount = 0;
                    _.each($scope.resource.background.data.backgrounds, function(background, k) {
                        if (background.cat.length == 0) background.cat = [0];
                        if ( _.includes(background.cat, cat.id) || _.includes(background.cat, "" + cat.id) ) cat.amount++
                    });
                });
                $scope.resource.background.data.cat = _.sortBy($scope.resource.background.data.cat, 'id');
    ...
    $scope.changeCat = function(type, cat){
        ...
        switch(type){
            ...
            case 'templates':
                ...
                $scope.resource[type].items = [];
                $scope.resource[type].filter.totalPage = 0;
                $scope.loadTemplates();
                break;
        };
    };
    ...
    $scope.changePrintingOptions = function( first, initVariationSetting ){
        ...
        if( angular.isDefined( $scope.printOptions ) ){
            ...
            if( angular.equals( $scope.printOptions.odOption, $scope.lastPrintingOptions ) ){
                if( !initVariationSetting ){
                    if( angular.isUndefined(first) && angular.isDefined( $scope.printOptions.extraOdOption ) ){
                ...
                angular.copy( $scope.printOptions.odOption, $scope.lastPrintingOptions );
            }
            var data = $scope.printOptions.odOption;
            ...
            if( angular.isDefined( data.area ) ){
                $timeout(function(){
                    ...
                    if( data.area == 2 ){
                        $scope.printOptions.extraOdOption.shape = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 ' + width + ' ' + height + '" ><path fill="rgba(0,0,0,0.001)" d="M0 ' + height/2 + 'A ' + width/2 + ' ' + height/2 + ' 0 0 0 ' + width + ' ' + height/2 + ' A ' + width/2 + ' ' + height/2 + ' 0 0 0 0 ' + height/2 + 'z"/></svg>';
                    }else{
                        $scope.printOptions.extraOdOption.shape = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 ' + width + ' ' + height + '" ><path fill="rgba(0,0,0,0.001)" d="M0 0 H'+width+' V'+height+' H0 V0z"/></svg>';
                    }
                    $scope.changeExtraOdOptions();
                }, 1000);
            }
    ...
    $scope.changeExtraOdOptions = function(){
        if( $scope.onloadVariation ) return;
        if( angular.isDefined( $scope.printOptions ) && angular.isDefined( $scope.printOptions.extraOdOption ) ){
            var data = $scope.printOptions.extraOdOption;
    ...
    $scope.uploadFile = function(files, indexFile){
        ...
        if( file.type.indexOf("svg") > -1 ){
                ...
                uploadEventHandlers: {
                    progress: function (e) {
                        if (e.lengthComputable) {
                            ...
                            $scope.resource.upload.progressBar = progressBar.toFixed(0);
                        }
                    }
                }
            };
            ...
            $http({
                method: 'POST',
                url: NBDESIGNCONFIG.apiUrl + '/files/shop',
                headers: {
                    Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
                },
                data: {
                    ...
                    resourceType: $scope.isTemplateMode ? 'templates' : 'images',
                    fileSize: file.size,
                }
            }).then(function successCallback(response){
                if( angular.isDefined( response.data ) ){
                    const { key, data: presignedPostData } = response.data;
                    ...
                    $http.post(presignedPostData.url, formData, config).then(
                        function(response) {
                            $http({
                                method: 'POST',
                                url: NBDESIGNCONFIG.apiUrl + '/files/shop/thumbnail',
                                headers: {
                                    Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
                                },
                                data: {
                                ...
                                if( angular.isDefined( response.data ) ){
                                    const data = JSON.parse(response.data.Payload.replace(/\\/g, ''));

                                    $scope.resource.upload.ilr = (!NBDESIGNCONFIG.nbdesigner_enable_low_resolution_image && NBDESIGNCONFIG.nbdesigner_mindpi_upload > data.density) ? true : false;
                                    
                                    if( $scope.resource.upload.ilr && !NBDESIGNCONFIG.nbdesigner_enable_low_resolution_image ){
                                        $scope.toggleStageLoading();
                                        alert(data.mes);
                                        $scope.uploadFile(files, indexFile + 1);
                                        return;
                                    ...
                                    data.origin_url = `${NBDESIGNCONFIG.baseUrl}/${key}`;
                                    data.src = `${NBDESIGNCONFIG.baseUrl}/${data.dstKey}`;
    ...
    $scope.fetchImage = function(imageUrl){
        $scope.toggleStageLoading();
        var stage = $scope.stages[$scope.currentStage],
            _canvas = stage['canvas'],
            object = _canvas.getActiveObject();
        ...
        $http({
            method: 'POST',
            url: NBDESIGNCONFIG.apiUrl + '/files/shop/fetch',
            headers: {
                Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
            },
            data: {
    ...
    $scope.storeUploadFile = function(data, name){
        ...
        if( typeof data == 'object' && angular.isDefined( data.origin_url ) ){
            obj.origin_url      = data.origin_url;
            obj.src             = src;
            obj.width           = data.width;
            obj.height          = data.height;

            if( angular.isDefined( data.origin_pdf ) ){
                obj.origin_pdf  = data.origin_pdf;
            }
        }

        $scope.resource.upload.data.push( obj );
        if( $scope.resource.upload.data.length > 20 ) $scope.resource.upload.data.shift();

        if( $scope.settings.nbdesigner_cache_uploaded_image ){
            $scope._localStorage.insert('od-uploaded', obj);
        }
    };
    ...
    $scope.takeSnapshot = function(){
        Webcam.snap( function(data_uri) {
            ...
            const file = $scope.makeblob(data_uri);
            ...
            $http({
                ...
                if( angular.isDefined( response.data ) ){
                    ...
                    $http.post(presignedPostData.url, formData, config).then(
                        function(response) {
                            $scope.toggleStageLoading();
                            $scope.addImage(`${NBDESIGNCONFIG.baseUrl}/${key}`, false, true);
                        },
                        function(response) {
                            console.log(response);
                            $scope.toggleStageLoading();
    ...
    $scope.generateTypoLink = function(typo){
        return NBDESIGNCONFIG.typographyUrl + 'designs/' + typo.folder + '/preview.png';
    };
    /* Fonts */
    ...
    $scope.saveData = async function(type){
        ...
        async function _saveData (){
            ...
            $scope.resource.config.dpi = NBDESIGNCONFIG.nbdesigner_default_dpi;
            $scope.resource.config.unit = NBDESIGNCONFIG.nbdesigner_dimensions_unit;
            ...
            var dataObj = {};
            const folder = designUuid();
            const promises = [];
            const resourceType = $scope.isTemplateMode ? 'templates' : 'designs';

            const createPathPromise = () => {
                return new Promise((resolve, reject) => {
                    $http({
                        method: 'POST',
                        url: NBDESIGNCONFIG.apiUrl + '/files/shop/path',
                        headers: {
                            Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
                        },
                        data: {
            ...
            const path = await createPathPromise();

            dataObj.used_font = new Blob([JSON.stringify($scope.resource.usedFonts)], {type: "application/json"});
            const usedFontsPromise = new Promise((resolve, reject) => {
                $http({
                    method: 'POST',
                    url: NBDESIGNCONFIG.apiUrl + '/files/shop/design',
                    headers: {
                        Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
                    },
                    data: {
                        contentType: "application/json",
                        fileName: 'used_font.json',
                        resourceType,
                        fileSize: dataObj.used_font.size,
                        path,
                    }
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
            ...
            promises.push(usedFontsPromise);

            if( $scope.isTemplateMode ) $scope.resource.jsonDesign.canvas = {
                width: $scope.settings.product_data.product[0].product_width, 
                height: $scope.settings.product_data.product[0].product_height
            };
            dataObj.design = new Blob([JSON.stringify($scope.resource.jsonDesign)], {type: "application/json"});
            const designPromise = new Promise((resolve, reject) => {
                $http({
                    method: 'POST',
                    url: NBDESIGNCONFIG.apiUrl + '/files/shop/design',
                    headers: {
                ...
                const svgPromise = new Promise((resolve, reject) => {
                    $http({
                        ...
                        data: {
                            ...
                            fileName: `${svg_key}.svg`,
                            resourceType,
                            fileSize: dataObj[svg_key].size,
                            path,
                        }
                    }).then(function successCallback(response){
                        if( angular.isDefined( response.data ) ){
                ...
                await Promise.all(promises).then(async (responses) => {
                    ...
                    responses.forEach((response, index) => {
                        const promise = new Promise((resolve, reject) => {
                            const { key, data: presignedPostData } = response;
                            const formData = new FormData();
                            let file;
                            Object.keys(presignedPostData.fields).forEach((key) => {
                                formData.append(key, presignedPostData.fields[key]);
                            ...
                            if( index == 0 ) file = dataObj.used_font;
                            if( index == 1 ) file = dataObj.design;
                            if( index == 2 ) file = dataObj.config;
                            if( index > 2 ){
                                if( index % 2 == 1 ){
                                    file = dataObj['frame_' + (index - 3 - (index - 3) / 2)];
                                } else {
                                    file = dataObj['frame_' + (index - 4 - (index - 4) / 2) + '_svg'];
                                }
                            }
                    ...
                    await Promise.all(uploadPromises).then((responses) => {
                        if( !$scope.isTemplateMode ){
                            const url = $scope.resource.savedDesign ? 
                                ( NBDESIGNCONFIG.apiUrl + '/designs/shop/' + $scope.resource.designUUID ) : 
                                NBDESIGNCONFIG.apiUrl + '/designs';
                            const method = $scope.resource.savedDesign ? 'PATCH' : 'POST';
                            ...
                            $http({
                                ...
                                data: {
                                    path,
                                    numberOfSide: $scope.stages.length,
                                    download: ( type == 'download-pdf' || type == 'download-jpg' ) ? true : false
                                }
                            }).then(function successCallback(response){
                                if( angular.isDefined( response.data ) ){
                                    nbd_window.postMessage({
                                        type: 'onSavedDesign',
                                        path,
                                        uuid: $scope.resource.savedDesign ? $scope.resource.designUUID : response.data.uuid,
                                        numberOfSide: $scope.stages.length
                                    }, '*');

                                    if( !$scope.resource.savedDesign ) $scope.resource.designUUID = response.data.uuid;
                                    $scope.resource.savedDesign = true;

                                    if( type == 'download-pdf' || type == 'download-jpg' ){
                                        const url = type == 'download-pdf' ? `${NBDESIGNCONFIG.apiUrl}/resources/shop/pdf/` + $scope.resource.designUUID
                                            : `${NBDESIGNCONFIG.apiUrl}/resources/shop/image/` + $scope.resource.designUUID + `/jpg`;
                                        $http({
                                            method: 'GET',
                                            url,
                                            headers: {
                                        ...
                                        nbd_window.postMessage("onCloseEditor", '*');
                                        $scope.toggleStageLoading();
                                    }
                                } else {
                                    console.log(response);
        ...
        if( $scope.onSaveData && NBDESIGNCONFIG.nbdesigner_enable_font_to_outlines ){
    ...
    $scope.loadOutlineFonts= function(){
        _.each($scope.stages, function(stage, index){
            ...
            _canvas.forEachObject(function(obj, objIndex) {
                if( obj.type == 'i-text' || obj.type == 'textbox' || obj.type == 'text' || obj.type == 'curvedText' ){
                    ...
                    if( font.type != 'google' ){
                        font_url = NBDESIGNCONFIG.baseUrl + '/' + font_url;
                    }else{
        ...
        $scope.outlineFonts.forEach(function(font){
            const func = /\.woff2$/.test(font.url) ? loadSvgFont : opentype.load;
            func( font.url, function (err, fontJson) {
                if( err ){
                    font.error = true;
                }else{
                    font.loaded = true;
    ...
    $scope.downloadDesign = function( downloadType ){
        if( NBDESIGNCONFIG.nbdesigner_enable_font_to_outlines ){
            $scope.loadOutlineFonts();
            var status = $scope.checkLoadedOutlineFont();
            if( status.error ){
                alert( NBDESIGNCONFIG.nbdlangs.wrong_to_convert_outline_font );
    ...
    $scope.saveDesign = function( downloadType ){
        ...
        _.each($scope.stages, function(stage, index){
            ...
            if( stage.config.use_svg_overlay && angular.isDefined( stage.svgOverlays ) && stage.svgOverlays.originDesignPaths.length && !downloadType ){
                $timeout(function(){
                    var _viewBox = stage.svg.match(/viewBox="(.*?)"/)[1],
                    viewBox = _viewBox.split(/\s+|,/);

                    var reg = /d=["'](.*?)["']/g;
                    stage.svgOverlays.designPaths = [];
                    stage.svgOverlays.originDesignPaths.forEach(function (path) {
                        var mat = reg.exec( path );
                        if( mat ){
                            var pathStr = mat[1];
                            var scaledPath = $scope.processSvgOverlayPath( pathStr, viewBox[2], stage.svgOverlays.viewBoxWidth, index );
                            stage.svgOverlays.designPaths.push( path.replace(reg, 'd="' + scaledPath + '"') );
                        }
                    });

                    var clipPath = '<clipPath id="svg-overlay-' + index + '">' + 
                        stage.svgOverlays.designPaths.join('')
                        + '</clipPath>';
                    var newSvg = stage.svg.replace('<\/defs>', clipPath + '<\/defs><g clip-path="url(#svg-overlay-' + index + ')">' );
                    newSvg = newSvg.replace('<\/svg>', '<\/g><\/svg>');
                    stage.svg = newSvg;
                    $scope.stageSaveStatus[index] = true;
                });
            } else if( angular.isDefined( $scope.areaDesignZoomedClipPaths[index] ) && $scope.areaDesignZoomedClipPaths[index] && !downloadType ){
                $timeout(function(){
                    ...
                    $timeout(function(){
                        ...
                        if( downloadType == 'png' ){
                            ...
                            if( angular.isDefined( $scope.areaDesignZoomedClipPaths[index] ) && $scope.areaDesignZoomedClipPaths[index] ){
                                ...
                                newSvg = newSvg.replace('<\/svg>', '<\/g><\/svg>');
                                stage.svg = newSvg;
                            } else if( stage.config.use_svg_overlay && angular.isDefined( stage.svgOverlays ) && stage.svgOverlays.originDesignPaths.length ){
                                var _viewBox = stage.svg.match(/viewBox="(.*?)"/)[1],
                                viewBox = _viewBox.split(/\s+|,/);
                            ...
                            let _svg = stage.svg;
                            let regex = /(xlink:href="[^&]+\?[^&]+(?:&.+=[^"]+){1,}")/g; 
                            const match = _svg.match(regex);
                            if(match && match.length){
                              for(let i = 0; i < match.length; i++){
                                  const newMatch = match[i].replace(/&/g, '&amp;');
                                  _svg = _svg.replace(match[i], newMatch);
                              }
                            }

                            var url = "data:image/svg+xml;charset=utf-8," + encodeURIComponent( _svg );
    ...
    $scope.onClickStage = function( $event ){
        ...
        if(angular.element($event.target).hasClass('stage')){
            $scope.deactiveAllLayer();
            /* store all stages */    
            if( $scope.settings.nbdesigner_save_latest_design ){
                $scope.saveDesign();
                var json = {config: {}};
                ...
                var pid = NBDESIGNCONFIG['product_id'];
    ...
    $scope.selectGlobalPicker = function(color){
        ...
        if( attr_arr.length == 2 && attr_arr[0] == 'text' ){
            ...
            $timeout(function(){
                if(attr_arr[1] == 'stroke') jQuery('.menu-item.item-stroke').addClass('active');
            });
    ...
    $scope.$on('nbd:picker', function(event, attr, color){
        ...
        $scope.globalPicker.color = color;
        $scope.globalPicker.active = true;
        $scope.onChangeRgbColorValue(color);
    });
    ...
    $scope.loadTemplateAfterRenderCanvas = function(){
        ...
        if( !($scope.settings.task == 'create' && !$scope.preventChangeCurrentDesign) ){
            $timeout(function(){
                function loadTemplate(){
                    if( NBDESIGNCONFIG.product_data.option.admindesign || $scope.settings.currentTemplate ){
                        if( NBDESIGNCONFIG.task == 'edit' ){
                            $scope.boostInsertTemplate(false, $scope.settings.currentTemplate);
                        } else {
                            if( $scope.settings.currentTemplate ){
                                $scope.boostInsertTemplate(false, $scope.settings.currentTemplate);
                            } else {
                                var interval = setInterval(function () {
                                    if ($scope.resource.templates.fetched) {
                                        if( $scope.resource.templates.items.length > 0 ){
                                            $scope.boostInsertTemplate(false, $scope.resource.templates.items[0]);
                                        }else{
                                            $scope.initStagesSettingWithoutTemplate();
                                        };
                    ...
                    if( $scope.settings.nbdesigner_save_latest_design ){
                        if($scope.localStore.ready){
                            $scope.localStore.get(pid, function(data){
                                if(data){
                                    const expired = new Date().getTime() - 30 * 24 * 60 * 60 * 1e3;
                                    if(data.createdAt && data.createdAt > expired){
    ...
    $scope.calcViewport = function(){
        var _offsetWidth = checkMobileDevice() ? 20 : 100,
            _offsetHeight = checkMobileDevice() ? 70 : 200,
        ...
        if( navigator.userAgent.indexOf("Safari") != -1 && navigator.userAgent.indexOf("CriOS") == -1 ){
            _offsetHeight = checkMobileDevice() ? 60 : 200;
    ...
    $scope.processProductSettings = function(){
        var unitRatio = 1;
        const unit = $scope.settings.nbdesigner_dimensions_unit;
        switch( unit ){
            case 'in':
                unitRatio = 2.54;
                break;
        ...
        _.each($scope.settings.product_data.product, function(side, index){
            ...
            $timeout(function(){
                ...
                if( side.use_svg_overlay == '1' ){
                    $scope.processSvgOverlay( index, side );
                }else{
                    $scope.stages[index].config.use_svg_overlay = false;
                }
            });
        });
        ($scope.settings.showRuler && appConfig.isModern) && $scope.initRuler();
    };
    $scope.processSvgOverlay = function( stageIndex, side ){
        var stage = $scope.stages[stageIndex];
        stage.config.use_svg_overlay = true;
        stage.svgOverlays = {
            design: side.design_svg_overlay,
            originDesignPaths: [],
            designPaths: [],
            cutline: side.cutline_svg_overlay,
            safezone: side.safezone_svg_overlay
        };
        jQuery('#stage-container-' + stageIndex + ' .stage-overlay-wrap svg').remove();
        if( side.design_svg_overlay != '' ){
            var valid = false;
            var _viewBox = side.design_svg_overlay.match(/viewBox="(.*?)"/)[1],
            viewBox = _viewBox.split(/\s+|,/);
            stage.svgOverlays.viewBoxWidth = viewBox[2];
            var matches = side.design_svg_overlay.match(/<\s*path([^<]*)d=".*?"([^<]*)(?:<\s*\/path\s*>|\/\s*>)/gm);
            if( matches ){
                var reg = /d=["'](.*?)["']/g;
                matches.forEach(function (match) {
                    var mat = reg.exec( match );
                    if( mat ){
                        var pathStr = mat[1];
                        stage.svgOverlays.originDesignPaths.push( match );
                        var scaledPath = $scope.processSvgOverlayPath( pathStr, stage.config.cwidth, viewBox[2], stageIndex );
                        stage.svgOverlays.designPaths.push( match.replace(reg, 'd="' + scaledPath + '"') );
                        valid = true;
                    }
                });
            }
            if( valid ){
                var clipPath = '<clipPath id="svg-overlay-' + stageIndex + '">' + 
                    stage.svgOverlays.designPaths.join('')
                    + '</clipPath>';
                var newSvg = stage.svgOverlays.design.replace(/(<svg(?:([^<]*))>)/gm, '$1' + '<defs>' + clipPath + '<\/defs><g style="opacity: 0" clip-path="url(#svg-overlay-' + stageIndex + ')">' );
                newSvg = newSvg.replace('<\/svg>', '<\/g><\/svg>');
                newSvg = newSvg.replace( '<svg', '<svg class="design_svg_overlay" ' );
                jQuery('#stage-container-' + stageIndex + ' .stage-overlay-wrap').append( newSvg );
            }else{
                stage.svgOverlays.design = '';
            }
        }
        if( side.cutline_svg_overlay != '' ){
            var cutline_svg = side.cutline_svg_overlay.replace( '<svg', '<svg class="cutline_svg_overlay" ' );
            jQuery('#stage-container-' + stageIndex + ' .stage-overlay-wrap').append( cutline_svg );
        }
        if( side.safezone_svg_overlay != '' ){
            var safezone_svg = side.safezone_svg_overlay.replace( '<svg', '<svg class="safezone_svg_overlay" ' );
            jQuery('#stage-container-' + stageIndex + ' .stage-overlay-wrap').append( safezone_svg );
        }
    };
    $scope.processSvgOverlayPath = function( pathStr, dstDiewboxWidth, viewboxWidth, stageIndex ){
        chunkArray = (arr, size = 2) => {
            let results = [];
            while (arr.length) {
        ...
        scalePath = (path, { scale = 1, scaleY, round = 3 } = {}) => {
        ...
        const scale = dstDiewboxWidth / viewboxWidth;
        const pathArr = parsePath( pathStr );
        const scaledPathArr = scalePath( pathArr, {scale});
        const scaledPath = stringifyPath( scaledPathArr );
    ...
    $scope.afterInsertTemplate = function(){
        ...
        _.each($scope.stages, function(stage, index){
            ...
            $timeout(function(){
                ...
                $scope.updateStageBackground(index);
            });
        });
    ...
    $scope.updateStageBackground = function(stageIndex) {
        var _stage = $scope.stages[stageIndex],
    ...
    $scope.onDrop = function(event){
        ...
        switch(type){
            ...
            case 'svg':
                if( extenal == 'true' ){
                    $scope.addArt({url: src}, true, true);
                    break;
                }
    ...
    $scope.onMouseUpStage = function(id, options){
        ...
        if( $scope.settings.nbdesigner_always_show_layer_action ){
            $scope.updateCorners();
        }
    ...
    $scope.onObjectAdded = function(id, options){
        ...
        if( $scope.contextAddLayers == 'normal' || $scope.contextAddLayers == 'copy' || $scope.contextAddLayers == 'template' ){
            ...
            if($scope.contextAddLayers == 'copy'){
                item.set({"itemId" : itemId});
            }
        };
    ...
    $scope.showFontsTab = function(){
        if( !checkMobileDevice() ){
            $timeout(function() {
                angular.element('#nav-fonts').triggerHandler('click');
            });
        };
    };
    ...
    $scope.onObjectModified = function(id, options){
        ...
        $scope.setHistory($scope.currentStage, true);
        if( $scope.stages[$scope.currentStage].states.isText && $scope.stages[$scope.currentStage].states.type != 'curvedText' && !$scope.settings.nbdesigner_enable_text_free_transform){
    ...
    $scope.updateTextPxFontSize = function (item) {
        ...
        if (textTypes.indexOf(itemType) > -1) {
            if (NBDESIGNCONFIG.nbdesigner_enable_text_free_transform == false) {
                var lastScaleX = item.scaleX,
    ...
    $scope.updateTextPxFontSize2 = function (item, ignoreMinMax) {
        if( !item || !item.get('itemId') ) return;
        if( $scope.stages[$scope.currentStage].states.isText && $scope.stages[$scope.currentStage].states.type != 'curvedText' && !$scope.settings.nbdesigner_enable_text_free_transform){
            var newFontSize = item.fontSize * item.scaleX;
            var lastScaleX = item.scaleX;
            var newPtFontSize = newFontSize / $scope.stages[$scope.currentStage].states.ratioConvertFont;
            var minSize = arrayMin($scope.listFontSizeInPt),
                maxSize = arrayMax($scope.listFontSizeInPt);
            if( $scope.forceMinSize && minSize > newPtFontSize && !ignoreMinMax ){
                newPtFontSize = minSize;
                newFontSize   = newPtFontSize * $scope.stages[$scope.currentStage].states.ratioConvertFont;
            }
            if( $scope.forceMaxSize && maxSize < newPtFontSize && !ignoreMinMax ){
                newPtFontSize = maxSize;
                newFontSize   = newPtFontSize * $scope.stages[$scope.currentStage].states.ratioConvertFont;
            }
            newPtFontSize = newPtFontSize.toFixed(2);
    ...
    $scope.onAfterRender = function(id, options){
        if( angular.isDefined( options.ctx ) ){
            var canvasId = options.ctx.canvas.id != '' ? options.ctx.canvas.id.split('-')[2] : id;
            var canvasEl = jQuery('#stage-container-' + canvasId + ' .lower-canvas')[0];
            $scope.stages[canvasId].stagePreview = canvasEl.toDataURL();
            id = canvasId;
        }
    ...
    $scope.onSelectionChanged = function(id, options){
        ...
        if( $scope.stages[$scope.currentStage].states.isGroup ){
            $scope.stages[$scope.currentStage].states.ilr = false;
        }else if( ($scope.stages[$scope.currentStage].states.isShape || $scope.stages[$scope.currentStage].states.isPath) && !NBDESIGNCONFIG.nbdesigner_clipart_rotate ){
        ...
        $scope.updateTextPxFontSize2( item, true );
    };
    $scope.onSelectionUpdated = function(id, options){
        ...
        var item = options.target;
        if( ($scope.stages[$scope.currentStage].states.isShape || $scope.stages[$scope.currentStage].states.isPath) && !NBDESIGNCONFIG.nbdesigner_clipart_rotate ){
    ...
    $scope.onTextChanged = function(id, options){
        ...
        if( item ){
            ...
            if( $scope.settings.nbdesigner_enable_text_check_lang ){
    ...
    $scope.canAddMoreLayer = function(){
        $check = true;
        if( $scope.settings.nbdesigner_prevent_add_more_layer && !$scope.isTemplateMode ) $check = false;
        return $check;
    }
    $scope.canDeleteLayer = function(){
        $check = true;
        if( $scope.settings.nbdesigner_prevent_delete_template_layer && !$scope.isTemplateMode ) $check = false;
        return $check;
    }
                ...
                switch(object.type) {
                    ...
                    case 'curvedText':
                        ...
                        if( !!object.isVertical ){
                            $scope.stages[$scope.currentStage].states.text.text = object.text.replace(/(\r\n|\n|\r)/gm, '');
                            $timeout(function(){
                                jQuery('.item-vertical').addClass('active');
                                if( $scope.settings.is_mobile ) jQuery('.main-toolbar').addClass('overflow-hidden');
                            }, 300);
                            $scope.verticalText.text = object.text.replace(/(\r\n|\n|\r)/gm, '');
                            $scope.verticalText.active = true;
                            $scope.stages[$scope.currentStage].states.text.isVertical = true;
                        } else {
                            $scope.stages[$scope.currentStage].states.text.isVertical = false;
                            $scope.verticalText.active = false;
                            $timeout(function(){
                                jQuery('.item-vertical').removeClass('active');
                            });
                        };
                        ...
                        $scope.stages[$scope.currentStage].states.enableRotate = $scope.settings.nbdesigner_text_rotate;
                        $scope.stages[$scope.currentStage].states.enableOpacity = $scope.settings.nbdesigner_text_opacity;
                        $scope.stages[$scope.currentStage].states.enableShadow = ( $scope.settings.nbdesigner_text_shadow && $scope.settings.nbdesigner_enable_cloud2print_api ) ? true : false;
                        break;
                    case 'image':
                    case 'custom-image':
                        ...
                        ['crop_left', 'crop_top', 'crop_width', 'crop_height', 'crop_scaleX', 'crop_scaleY'].forEach(function(key){
                            if( angular.isDefined(object[key]) ){
                                ...
                                delete $scope.stages[$scope.currentStage].states[key];
                            }
                        });
                    ...
                    case 'polyline':
                        ...
                        if( NBDESIGNCONFIG.nbdesigner_clipart_change_path_color ){
                        ...
                        $scope.stages[$scope.currentStage].states.enableRotate = NBDESIGNCONFIG.nbdesigner_clipart_rotate;
                        $scope.stages[$scope.currentStage].states.enableOpacity = NBDESIGNCONFIG.nbdesigner_clipart_opacity;
                        break;
                    case 'path-group':
                    case 'path':
                    case 'group':
    ...
    $scope.getFontInfo = function(alias, type){
        let font; let _font = {};
        if( type == 'google' ){
            font = _.filter(NBDESIGNCONFIG.googleFonts, { family: alias })[0];
        } else {
            font = _.filter($scope.resource.font.data, { alias: alias })[0];
        }
        _font = angular.copy(font, _font);
        if( type == 'google' ) {
            _font.subset = font.subsets[0];
        }

        if(!_font){
            _font = {name: 'Roboto', alias: 'Roboto', file: {r: 1, b: 1, i: 1, bi: 1}, cat: [-1], type:"google",subset:"latin"};
        }
        return _font;
    };
    ...
    $scope.insertTemplateFont = function(font_name, callback, type){
        var fontName = font_name;
        var font_id = fontName.replace(/\s/gi, '').toLowerCase();
        var _font = $scope.getFontInfo(font_name, type);
        if( type == 'google' || _font.type == 'google' ){
            ...
            if( !jQuery('#' + font_id).length ){
                ...
                _.each(_font.file, function (file, index) {
                    if( file != 0 ){
                        var font_url = NBDESIGNCONFIG.baseUrl + '/' + file;
    ...
    $scope.boostInsertTemplate = async function(local, temp){
        $scope.currentLocalTempId = temp.uuid;
        ...
        async function loadDesign (fonts, design, viewport, konfig){
            async function loadStages(){
                async function loadLayer(object, objectIndex, stageIndex){
                    ...
                    if( type == 'image' || type == 'custom-image' ){
                        ...
                        if( angular.isDefined(object.v_card) && !$scope.isTemplateMode ){
                    ...
                    $scope.renderTextAfterLoadFont(layers, function(){
                        ...
                        $timeout(function(){
                            ...
                            if( index == $scope.stages.length - 1 ){
                                $scope.onloadTemplate = false;
                                $scope.contextAddLayers = 'normal';
                                if( angular.isDefined(design.canvas) ){
                                    viewport = {
                                        ...viewport,
                                        templateWidth: design.canvas.width,
                                        templateHeight: design.canvas.height
                                    };
                                    $scope.resizeStages(viewport);
                                } else if( angular.isDefined(viewport) ){
                                    $scope.resizeStages(viewport);
                                }else{
                                    $scope.toggleStageLoading();
                                }
            ...
            function loadFont( fontAlias, cb ){
                return new Promise(resolve => {
                    $scope.insertTemplateFont(fontAlias, function(){
                        cb();
                        resolve('success')
                    });
                });
            ...
            if(fonts.length){
                ...
                _.each(fonts, function(font, index){
                    ...
                    promises.push(loadFont(font.alias, () => {
        ...
        if( local ){
            ...
            const usedFontsPromise = new Promise((resolve, reject) => {
                $http({
                    method: 'GET',
                    url: `${NBDESIGNCONFIG.baseUrl}/${temp.path}used_font.json`,
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
                        resolve(response.data)
                    }
            ...
            const designPromise = new Promise((resolve, reject) => {
                $http({
                    method: 'GET',
                    url: `${NBDESIGNCONFIG.baseUrl}/${temp.path}design.json`,
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
                        resolve(response.data)
                    }
            ...
            const configPromise = new Promise((resolve, reject) => {
                $http({
                    method: 'GET',
                    url: `${NBDESIGNCONFIG.baseUrl}/${temp.path}config.json`,
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
                        resolve(response.data)
                    }
            ...
            await Promise.all(promises).then(([fonts, design, config]) => {
                loadDesign(fonts, design, config.viewport, config);
            }, function (err) {
                console.log(err);
                $scope.toggleStageLoading();
            });
    ...
    $scope.insertPartTemplate = function(temp, partIndex){
        var stage = $scope.stages[$scope.currentStage],
        _canvas = stage['canvas'],
        layers = _canvas.getObjects();
        if( layers.length > 0 ){
            $scope._currentTemp = temp;
            $scope._currentpartIndex = partIndex;
            jQuery('.nbd-popup.insert-part-template-alert').nbShowPopup();
        }else{
            $scope._insertPartTemplate(temp, partIndex);
        }
    };
    $scope.closePopupInsertPartTem = function(){
        jQuery('.insert-part-template-alert .close-popup').triggerHandler('click');
    };
    $scope.closePopupInsertGlobalPartTem = function(){
        jQuery('.insert-part-global-template-alert .close-popup').triggerHandler('click');
    };
    $scope._insertPartTemplate = async function(temp, partIndex){
        $scope.onloadTemplate = true;
        $scope.contextAddLayers = 'template';
        $scope.toggleStageLoading();
        jQuery('.insert-part-template-alert .close-popup').triggerHandler('click');
    ...
    $scope.resizeStage = function(viewport, stage_id, wihoutLoading){
        ...
        if( viewport.templateWidth ){
            var newTemplateFitRec = $scope.fitRectangle(stage.config.width, stage.config.height, viewport.templateWidth, viewport.templateHeight, true);
            var oldTemplateFitRec = $scope.fitRectangle(viewport.width, viewport.height, viewport.templateWidth, viewport.templateHeight, true);

            var factor = newTemplateFitRec.width / oldTemplateFitRec.width;
            ...
            stage.canvas.forEachObject(function (obj) {
                var scaleX = obj.scaleX,
                    ...
                    tempLeft = left * factor + newTemplateFitRec.left,
                    tempTop = top * factor + newTemplateFitRec.top;
    ...
    $scope.insertTypography = async function(typo){
        ...
        const getDesign = new Promise((resolve, reject) => {
            setTimeout(() => {
                $http({
                    method: 'GET',
                    url: `${NBDESIGNCONFIG.typographyUrl}designs/${typo.folder}/design.json`,
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
                        resolve(response.data)
                    }
        ...
        const getUsedFonts = new Promise((resolve, reject) => {
            setTimeout(() => {
                $http({
                    method: 'GET',
                    url: `${NBDESIGNCONFIG.typographyUrl}designs/${typo.folder}/used_font.json`,
                }).then(function successCallback(response){
                    if( angular.isDefined( response.data ) ){
                        resolve(response.data)
                    }
        ...
        await Promise.all([getDesign, getUsedFonts]).then(([design, _fonts]) => {
            const fonts = _fonts.map(font => {
                const _font = { ...font };

                const googleFont = _.filter(NBDESIGNCONFIG.googleFonts, { family: font.alias })[0];

                if(googleFont && googleFont.file){
                    _font.file = googleFont.file;
                } else {
                    for (const [key, value] of Object.entries(font.file)) {
                        _font.file[key] = value;
                    }
                }

                delete _font.$$hashKey;
                _font.cat = -1;

                return _font;
            });
            const layers = design.frame_0.objects;
    ...
    $scope.renderTextAfterLoadFont = async function(layers, callback ){
        if( layers.length == 0 ) {
            callback();
            return;
        };
        const promises = [];
        function loadFont( font, preview_text, cb ){
            return new Promise((resolve, reject) => {
                font.load(preview_text).then(function () {
                    cb()
                    resolve('success');
                }, function () {
                    reject('fail');
        ...
        _.each(layers, function(item, index){
            if( ['text', 'i-text', 'curvedText', 'textbox'].indexOf(item.type) > -1 ){
                ...
                promises.push(loadFont(font, $scope.settings.subsets[_font.subset]['preview_text'], () => {
                    fabric.util.clearFabricFontCache();
                    item.initDimensions();
                    item.setCoords();
                }))
                ...
                reader.onload = function(event){
                    if (event.target.readyState === 2) {
                        ...
                        if( result.config.version == 3 ){
                            var temp = {fonts: result.fonts, design: result.design, viewport: result.config.viewport};
                            $scope.insertTemplate(true, temp);
                        } else {
                            alert('Can not import the external design!')
                        }
                        destroy();
                    }
                };
    ...
    $scope.deleteLayers = function(itemId){
        if( !$scope.canDeleteLayer() ) return;
        var _canvas = this.stages[$scope.currentStage]['canvas'];
        var item = angular.isDefined(itemId) ? _canvas.item($scope.getLayerById( itemId )) : _canvas.getActiveObjects();
        if(item){
    ...
    $scope.activeLayer = function(itemId, layerIndex){
        var _stage = $scope.stages[$scope.currentStage],
        _canvas = _stage['canvas'];
        if( !$scope.isTemplateMode && angular.isDefined( _canvas.item($scope.getLayerById( itemId )).forceLock ) && _canvas.item($scope.getLayerById( itemId )).forceLock ) return;
        _canvas.setActiveObject(_canvas.item($scope.getLayerById( itemId )));
        const _layerIndex = $scope.stages[$scope.currentStage].layers.findIndex(layer => layer.itemId == itemId);
        if( angular.isDefined(layerIndex) ) _stage.layers[_layerIndex].active = true;
        $scope.renderStage();
    };
    ...
    $scope._addStage = function( stage_id ){
        ...
        $scope.stages[stage_id * 1 + 1].stagePreview = false;
    };
    ...
    $scope.getGridViewStyle = function($index, type, prop){
        var val = productWidth = productHeight = productLeft = productTop = designWidth = designHeight = designtLeft = designTop = 0;
        if($scope.settings.product_data.product[$index].product_width > $scope.settings.product_data.product[$index].product_height){
            productWidth = 150;
            productHeight = 150 * $scope.settings.product_data.product[$index].product_height / $scope.settings.product_data.product[$index].product_width;
            productLeft = 0;
            productTop = 75 * (1- $scope.settings.product_data.product[$index].product_height / $scope.settings.product_data.product[$index].product_width);
        } else {
            productHeight = 150;
            productWidth = 150 * $scope.settings.product_data.product[$index].product_width / $scope.settings.product_data.product[$index].product_height;
            productLeft = 75 * (1- $scope.settings.product_data.product[$index].product_width / $scope.settings.product_data.product[$index].product_height);
            productTop = 0;
        }


        if(type == 'background'){
            if(prop == 'width') val = productWidth;
            if(prop == 'height') val = productHeight;
            if(prop == 'left') val = productLeft;
            if(prop == 'top') val = productTop;
        }

        if(type == 'design'){
            if(prop == 'width') val = ($scope.settings.product_data.product[$index].real_width / $scope.settings.product_data.product[$index].product_width) * productWidth;
            if(prop == 'height') val = ($scope.settings.product_data.product[$index].real_height / $scope.settings.product_data.product[$index].product_width) * productWidth;
            if(prop == 'left') val = productLeft + ($scope.settings.product_data.product[$index].real_left / $scope.settings.product_data.product[$index].product_width) * productWidth;
            if(prop == 'top') val = productTop + ($scope.settings.product_data.product[$index].real_top / $scope.settings.product_data.product[$index].product_width) * productWidth;
        }
    ...
    $scope.setCanvasBackground = function(background){
        if(background.type == 'color'){
            $scope.changeBackgroundCanvas(background.color);
        }else{
            var _stage = $scope.stages[$scope.currentStage],
            _canvas = _stage.canvas,
            scale = _stage.states.scaleRange[_stage.states.currentScaleIndex].ratio;
            ...
            try {
                function _setCanvasBackground(item, isSvg) {
                    _canvas.backgroundColor = null;
                    _canvas.forEachObject(function(obj) {
                        if( obj.isBgImg ){
                            _canvas.remove(obj);
                    ...
                    if( isSvg ){
                        item.scaleToWidth(new_width);
                        item.scaleToHeight(new_height);
                    } else {
                        item.set({
                            scaleX: new_width / item.width,
                            scaleY: new_height / item.height,
                    ...
                    _canvas.viewportCenterObject(_item);
                    _item.sendToBack();
                    $scope.deactiveAllLayer();
                    _canvas.renderAll();
                    $scope.afterObjectModify();
                ...
                if( background.url.toLowerCase().endsWith('.svg') ){
                    fabric.loadSVGFromURL(background.url, function(ob, op) {
                        if(ob){
                            const item = fabric.util.groupSVGElements(ob, op);
                            _setCanvasBackground(item, true);
                        }else{
                            console.log('Try again!');
                        }
                    });
                } else {
                    fabric.Image.fromURL(background.url, function(item) {
                        _setCanvasBackground(item);
                    }, {crossOrigin: 'anonymous'});
                }
            } catch(err) {
                $scope.toggleStageLoading();
    ...
    $scope.showBrushColorPicker = false;
    $scope.isValidRgbColor = function(color){
        const _color = tinycolor(color);
        return color && color.length === 7 && _color.isValid();
    };
    $scope.isValidCmykValue = function(value) {
        return Number.isInteger(value) && value >= 0 && value <= 100;
    };
    $scope.updateCurrentRgbColor = debounce(function() {
        $http({
            method: 'POST',
            url: `${NBDESIGNCONFIG.wslApiUrl}/color-palettes/shop/cmyk2rgb?token=${NBDESIGNCONFIG.clientToken}`,
            headers: {
                'Accept': 'application/json'
            },
            data: {
                c: $scope.currentColorInfo.c,
                m: $scope.currentColorInfo.m,
                y: $scope.currentColorInfo.y,
                k: $scope.currentColorInfo.k,
            }
        }).then(function successCallback(response){
            if( angular.isDefined( response.data ) ){
                const hex = rgb2hex(response.data[0], response.data[1], response.data[2]);
                $scope.currentColorInfo.rgb = hex;
                $scope.updateColorPickerValue(hex);
                if( $scope.stageBgColorPicker.status ){
                    $scope.stageBgColorPicker.currentColor = hex;
                }
                if($scope.globalPicker.active){
                    $scope.globalPicker.color = hex;
                }
                if($scope.resource.drawMode.showColorPicker){
                    $scope.resource.drawMode.brushColor = hex;
                }
            }
        }, function errorCallback(response) {
            console.log('Fail get rgb color!');
            $scope.updateColorPickerValue($scope.currentColorInfo.rgb);
            if( $scope.stageBgColorPicker.status ){
                $scope.stageBgColorPicker.currentColor = $scope.currentColorInfo.rgb;
            }
            if($scope.globalPicker.active){
                $scope.globalPicker.color = $scope.currentColorInfo.rgb;
            }
            if($scope.resource.drawMode.showColorPicker){
                $scope.resource.drawMode.brushColor = $scope.currentColorInfo.rgb;
            }
        });
    }, 300);
    $scope.updateCurrentCmykColor = debounce(function(){
        const rgb = hex2rgb($scope.currentColorInfo.rgb.replace('#', ''));
        $http({
            method: 'POST',
            url: `${NBDESIGNCONFIG.wslApiUrl}/color-palettes/shop/rgb2cmyk?token=${NBDESIGNCONFIG.clientToken}`,
            headers: {
                'Accept': 'application/json'
            },
            data: {
            ...
            if( angular.isDefined( response.data ) ){
                $scope.currentColorInfo.c = response.data[0];
                $scope.currentColorInfo.m = response.data[1];
                $scope.currentColorInfo.y = response.data[2];
                $scope.currentColorInfo.k = response.data[3];
            }
        }, function errorCallback(response) {
            console.log('Fail get cmyk color!');
        });
    });
    $scope.updateCurrentCmykColor();
    $scope.updateColorPickerValue = function(color){
        if( !$scope.isValidRgbColor(color) ) return;
        $scope.currentColor = color;
    };
    $scope.updateCurrentColor = function(color){
        if( !$scope.isValidRgbColor(color) ) return;
        const index = $scope.listInfoAddedColor.findIndex(c => c.rgb.toLowerCase() == color.toLowerCase());
        if( index > -1 ){
            $scope.currentColorInfo = {
                rgb: color.toLowerCase(),
                c: $scope.listInfoAddedColor[index].c,
                m: $scope.listInfoAddedColor[index].m,
                y: $scope.listInfoAddedColor[index].y,
                k: $scope.listInfoAddedColor[index].k,
            }
        } else {
            const rgb = hex2rgb(color.replace('#', ''));
            ...
            $scope.updateCurrentCmykColor(color);
        }
    };
    $scope.onChangeRgbColorValue = function(color){
        if( !$scope.isValidRgbColor(color) ) return;
        if( !$scope.currentColorInfo || !$scope.listInfoAddedColor ) return;
        if( $scope.currentColorInfo.rgb.toLowerCase() == color.toLowerCase() ) return;
        $scope.updateCurrentColor(color);
    };
    $scope.onChangeCmykColorValue = function(){
        if(!['c', 'm', 'y', 'k'].reduce((acc, cur) => acc && $scope.isValidCmykValue($scope.currentColorInfo[cur]), true)) return;
        const rgb = cmyk2rgb($scope.currentColorInfo.c, $scope.currentColorInfo.m, $scope.currentColorInfo.y, $scope.currentColorInfo.k, false);
        const hex = rgb2hex(rgb.r, rgb.g, rgb.b);
        $scope.currentColorInfo.rgb = hex;
        $scope.updateCurrentRgbColor();
    };
    $scope.updateListInfoAddedColor = function(){
        const index = $scope.listInfoAddedColor.findIndex(c => c.rgb.toLowerCase() == $scope.currentColorInfo.rgb.toLowerCase());
        if( index === -1 ){
            $scope.listInfoAddedColor.push($scope.currentColorInfo);
        }
    };
    ...
    $scope.addText = function(content, type, additionalObj){
        ...
        if(!_.filter(state.usedFonts, ['alias', fontName]).length){
            state.usedFonts.push($scope.getFontInfo(fontName));
        }
    };
    ...
    $scope.closeVerticalTextSettings = function($event){
        $scope.verticalText.active = false;
        if( $scope.settings.is_mobile ) jQuery('.main-toolbar').removeClass('overflow-hidden');
        $scope.updateApp();
        $timeout(function(){
    ...
    $scope.getVerticalTextContent = function(content){
        return content.replace(/(\r\n|\n|\r)/gm, '').split('').join('\n');
    };
    $scope.addVerticalText = function( content ){
        var state= $scope.stages[$scope.currentStage].states;
        ...
        function addText(){
            ...
            $scope.stages[$scope.currentStage]['canvas'].add(new FabricWindow['Text'](newContent, textObj));
        };
        var fontName = NBDESIGNCONFIG.default_font.alias,
        font = new FontFaceObserver(fontName);
        font.load($scope.settings.subsets[NBDESIGNCONFIG.default_font.subset]['preview_text']).then(function () {
    ...
    $scope.setLayerAttribute = function(type, value, itemId, layerIndex){
        var _canvas = $scope.stages[$scope.currentStage]['canvas'];
        if( !appConfig.ready ) return;

        var item = angular.isDefined(itemId) ? _canvas.item($scope.getLayerById( itemId )) : _canvas.getActiveObject();
        ...
        if(angular.isDefined(itemId)){
            const _layerIndex = $scope.stages[$scope.currentStage].layers.findIndex(layer => layer.itemId == itemId);
            $scope.stages[$scope.currentStage].layers[_layerIndex][type] = value;
        }else{
            if( type !== 'opacity' ) $scope.stages[$scope.currentStage].states[type] = value;
        }
        if( type != 'text' ){
            $scope.updateLayersList();
        }
        if( ['i-text', 'text', 'textbox', 'curvedText'].indexOf(itemType) > -1 ){
            $scope.normalizeText( item );
            if( $scope.settings.nbdesigner_enable_text_check_lang ) $scope.checkCharacter( item );
        }
        $scope.afterObjectModify();
        $scope.updateApp();
    };
    ...
    $scope.addImageFromUrl = function( dataImg, extenal, ilr, maskId ){
        ...
        if( url.includes('pixabay.com') ){
            $scope.fetchImage(url.toLowerCase());
            return;
        }
    ...
    $scope.addImage = function(data, showLoading, hideLoading, additionalTitle, additionalValue, maskId ){
        ...
        if( angular.isUndefined( maskId ) && !$scope.isTemplateMode && $scope.settings.nbdesigner_auto_fill_template_masks ){
            var _maskId = $scope.getEmptyMask();
            if( _maskId != '' ) maskId = _maskId;
        }
        if( angular.isUndefined( maskId ) && !$scope.isTemplateMode && $scope.settings.nbdesigner_limit_photo_by_masks ){
            if( hideLoading ) {
                $timeout(function(){
                    $scope.toggleStageLoading();
                });
        ...
        fabric.Image.fromURL(url, function(op) {
            if( angular.isDefined( maskId ) || $scope.stages[$scope.currentStage].states.elementUpload ){
                ...
                if( angular.isDefined( maskId ) ) {
                    ...
                    delete object.cropped;
                    delete object.origin_src;
                }
                object.set({
                    dirty: true,
                    width: op.width,
                ...
                var _ratio = NBDESIGNCONFIG.nbdesigner_enable_auto_fit_image ? 1 : 0.9;
    ...
    $scope.removeClippingMask = function(obj, needreRender, withoutRemoveMask){
        ...
        obj = obj ? obj : _canvas.getActiveObject();
        if( !!obj.get('maskId') ){
            var maskLayerIndex = $scope.getLayerById( obj.get('maskId') ),
            maskLayer = _canvas.item( maskLayerIndex );
        ...
        if( NBDESIGNCONFIG.nbdesigner_object_center_scaling == false ){
    ...
    $scope.cropImage = function(){
        ...
        $http({
            method: 'POST',
            url: NBDESIGNCONFIG.apiUrl + '/files/shop/crop',
            headers: {
                Authorization: `Bearer ${NBDESIGNCONFIG.clientToken}`
            },
            data: {
                startX: Math.floor(bound.left * imgScale),
                startY: Math.floor(bound.top * imgScale),
                width: Math.floor(bound.width * imgScale),
                height: Math.floor(bound.height * imgScale),
                path: ($scope.cropObj.origin_url ? $scope.cropObj.origin_url : $scope.cropObj.src).replace(`${NBDESIGNCONFIG.baseUrl}/`, ''),
                resourceType: $scope.isTemplateMode ? 'templates' : 'images'
            }
        }).then(function successCallback(response){
            if( angular.isDefined( response.data ) &&  response.data.Payload != 'null' ){
                add_image(`${NBDESIGNCONFIG.baseUrl}/${response.data.Payload}`);
            } else {
                jQuery('.nbd-popup.popup-nbd-crop').find('.overlay-main').removeClass('active');
            }
        }, function errorCallback(response) {
    ...
    $scope.addPhotoFrame = function( frame ){
        var url = NBDESIGNCONFIG.framesUrl + 'svgs/s' + ( frame + 1 ) + '.svg',
    ...
    $scope.showWelcomePopupStatus = false;
    $scope.showWelcomePopup = function(){
        var first_visitor = localStorage.getItem('showWelcomePopup');;
        if (!first_visitor && !$scope.isTemplateMode) {
            localStorage.setItem('showWelcomePopup', 1);
            $scope.showWelcomePopupStatus = true;
        }
    };
    ...
    $scope.show3DPreview = function(){
        ...
        function _show3DPreview(){
            if( !$scope.threeDimensionPreview.existedFrame ){
                var iframe_src = `${NBDESIGNCONFIG.apiUrl}/preview/${NBDESIGNCONFIG.cmsId}?token=${NBDESIGNCONFIG.clientToken}`;
    ...
    $scope.update3DPreviewTexture = function(){
        ...
        $scope.stages.map( stage => {
            var name = stage.config.name.trim();
            designs[name] = stage.design;
        });
        frame.contentWindow.postMessage(['update_design', designs], '*');
    };
    $scope.take3DPreviewScreenshot = function(){
        if( $scope.threeDimensionPreview.modelLoaded ){
            var frame = document.getElementById('preview-3d-wrap');
            frame.contentWindow.postMessage(['take_screenshot'], '*');
        }
    };
...
nbdApp.factory('FabricWindow', ['$window', function($window) {
    ...
    if( NBDESIGNCONFIG.nbdesigner_object_center_scaling ){
    ...
    if( NBDESIGNCONFIG.nbdesigner_enable_text_free_transform == false ){
        $window.fabric.IText.prototype.set({
            _controlsVisibility: {
                ...
                mr: false,
                mb: false,
                mtr: NBDESIGNCONFIG.nbdesigner_text_rotate
            }
        });
    }
    ...
    if ( NBDESIGNCONFIG.nbdesigner_enable_font_to_outlines ) {
        fabric.Text.prototype._createTextCharSpan = function (_char, styleDecl, left, top) {
            ...
            const checkCharacter = (text, fontJson) => {
                let check = true;

                var glyphs = fontJson.glyphs.glyphs;
                let arr = {};
                _.each(glyphs, function(glyph, index){
                    arr['g-' + glyph.unicode] = 1;
                });
            ...
            if( $window.outlineFonts[fontName] && checkCharacter(_char.replace(/\r?\n|\r/gm, '').replace(/\s/gm, ""), $window.outlineFonts[fontName]) ){
                var path = $window.outlineFonts[fontName].getPath(_char, toFixed(left, NUM_FRACTION_DIGITS), toFixed(top, NUM_FRACTION_DIGITS), this.fontSize)
        ...
        fabric.Text.prototype._wrapSVGTextAndBg = function (textAndBg) {
            ...
            const hasTspan = textAndBg.textSpans.some(span => /^<tspan/.test(span));
            let textComponent = [];
            if( hasTspan ){
                textComponent = [
                    ...
                    textAndBg.textSpans.filter(span => /^<tspan/.test(span)).join(''),
                    '</text>\n'
                ]
            }

            const hasPath = textAndBg.textSpans.some(span => /^<path/.test(span));
            let groupPathComponent = [];
            if( hasPath ){
                groupPathComponent = [
                    '\t\t<g ',
                    'style="', this.getSvgStyles(noShadow), '"', this.addPaintOrder(), ' >',
                    textAndBg.textSpans.filter(span => /^<path/.test(span)).join(''),
                    '</g>\n'
                ]
            }
...
nbdApp.directive('nbdScroll', ['$timeout', function($timeout){
    return {
        ...
        link: function( scope, element, attrs ) {
            $timeout(function() {
                var el = (scope.type != 'font' && scope.type != 'templates') ? jQuery(scope.container + ' .tab-scroll') : jQuery(scope.container),
...
nbdApp.filter("filterBackground", function() {
    return function(backgrounds, filterBackground) {
        ...
        // backgrounds = _.reverse(backgrounds);
        angular.forEach(backgrounds, function(background, key) {
            ...
            if( !!filterBackground.search ){
               check['name'] = background.name.toLowerCase().indexOf(filterBackground.search.toLowerCase()) >= 0 ? true : false;
            }else{
                check['name'] = true;
            };
            if( !!filterBackground.currentCat.id || filterBackground.currentCat.id === 0 ){
                check['cat'] = ( _.includes(background.cat, filterBackground.currentCat.id) || _.includes(background.cat, "" + filterBackground.currentCat.id) ) ? true : false;
            }else{
                check['cat'] = true;
            }
            if( check['limit'] && check['name'] && check['cat'] ) arrBackground.push(background);
        });
            ...
            if( !jQuery('#' + font_id).length ){
                if(fontType == 'google'){
                    ...
                    _.each(scope.font.file, function (file, index) {
                        var font_url = font_url = NBDESIGNCONFIG.baseUrl + '/' + file;
                        css += "@font-face {font-family: '" + fontName + "';";
                        css += "src: ";
                        css += "url('" + font_url + "') format('truetype');";
                        switch(index){
...
nbdApp.directive("nbdClearStage", function() {
    return {
        ...
        link: function(scope, element) {
            jQuery(element).on('click', function(){
                if( NBDESIGNCONFIG.nbdesigner_prevent_delete_template_layer && !( NBDESIGNCONFIG.task == 'create' || NBDESIGNCONFIG.task == 'edit' ) ) return;
                jQuery('.nbd-popup.clear-stage-alert, .v-popup.v-popup-select').nbShowPopup();
            });
        }
    };
...
if(NBDESIGNCONFIG.nbdesigner_show_all_template_sides){
...
function receiveMessage(event){
    ...
    if( typeof event.data == 'object'){
        if( event.data.type == 'change_nbo_options' ){
            scope.printOptions = event.data.printOptions;
            scope.changePrintingOptions();
        }

        if( event.data.type == "taked_screenshot" ){
            const filename = "3d-preview.png",
            a = document.createElement("a"),
            url = window.URL.createObjectURL(event.data.blob);
